...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default="netbeacon", type=str, help="netbeacon / planter / iisy")
    ...
    for dataset in ['ton-iot']:
        ...
        record_list_name = ['dataset','max_depth','model','add_order','choose_threshold','org_acc',\
                        'detection_acc','choose_TPR','choose_FPR','final_acc','precision' , 'recall' ,'f1','rule_num_ternary']
        ...
        # output_csv = "C2_ROC_"+str(args.test_split_size)+".csv"
        output_csv = "C2_netbeacon_iot_ROC_"+str(args.test_split_size)+".csv"
        ...
        for cls_threshold in x:
            ...
            for log_class in add_order: 
                ...
                # 用于记录
                experi_csv_order_list.append([args.dataset,args.max_depth,args.model,label_name[log_class],cls_threshold,org_acc,detection_acc,choose_TPR,choose_FPR,final_acc,precision , recall ,f1,num_ternary])
            ...
            # tp,fn,fp,tn对应相加
            detection_acc = 100*(new_cm[0]+new_cm[3])/sum(new_cm)
            experi_csv_order_list.append([args.dataset,args.max_depth,args.model,"ALL",cls_threshold,org_acc,detection_acc,choose_TPR,choose_FPR,ALL_final_acc,precision , recall ,f1,num_ternary])
            ...
            if os.path.exists(output_csv):
                with open(output_csv, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    # 写入超参数和结果
                    for experi_csv in experi_csv_order_list:
