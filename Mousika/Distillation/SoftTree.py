...
class SoftTreeClassifier:
    ...
    def predict_proba(self, X):
        if np.ndim(X) == 1:
            return self.__predict_proba_rec(X, self.root)
        else:
            result = []
            for sample in X:
                result.append(self.__predict_proba_rec(sample, self.root))
            return np.array(result)
    ...
    def __predict_proba_rec(self, X, node):
        if node.label is not None:
            total_samples = sum(node.label_dict.values())

            return [node.label_dict.get(i, 0) / total_samples for i in range(len(node.label_dict))]
            ...
            if feat_value is None or feat_value is np.nan:
                choice = np.random.randint(1, 3)
                if choice == 1:
                    return self.__predict_proba_rec(X, node.true_branch)
                else:
                    return self.__predict_proba_rec(X, node.false_branch)
            else:
                if feat_attr == 'd':
                    if feat_value == threshold:
                        return self.__predict_proba_rec(X, node.true_branch)
