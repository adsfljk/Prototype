...
if __name__ == '__main__':
    ...
    parser.add_argument('--export_rule', type=bool, default=True, help="Enable or disable the export_rule")
    ...
    for dataset in ["cicids-2018","ton-iot","unsw-nb15"]:
        ...
        output_csv = "appendix_ACC_mou.csv"
        ...
        record_list_name = ['dataset','add_order','choose_threshold','org_acc',\
                            'detection_acc','choose_TPR','choose_FPR','final_acc','rule_acc','precision' , 'recall' ,'f1','rule_number','time']


        # output_csv = args.teacher +str(args.min_sample_leaf)+"Mouresults"+str(args.attack_max_samples)+str(args.test_split_size)+".csv"
        ...
        for log_class in add_order:
            ...
            for cls_threshold in x:
                ...
                score = choose_TPR * (final_acc > 80) + args.tpr_acc_rate * final_acc
                if score > choose_threshold_tfpr:
                    choose_threshold_tfpr = score
                    end_time = time.time()
                    traing_time = end_time - start_time

                    experi_csv = [args.dataset,label_name[log_class],cls_threshold,org_acc,detection_acc,choose_TPR,choose_FPR,final_acc,rule_acc,precision , recall ,f1,rule_num,traing_time]
        ...
        end_time = time.time()
        traing_time = end_time - start_time
        experi_csv = [args.dataset,"ALL",cls_threshold,org_acc,detection_acc,choose_TPR,choose_FPR,final_acc,rule_acc,precision , recall ,f1,rule_num,traing_time]
