...
##################################################
    data_name = 'iscx'
    data = np.load("./ISCX_dataset/iscx_data_5class.npy")
    label = np.load("./ISCX_dataset/iscx_label_5class.npy")
    
    feature_max = np.max(data,axis=0)
    feature_min = np.min(data,axis=0)
    label = label.reshape(-1,1)

    X_train, X_test, y_train, y_test = train_test_split(data, label, test_size=0.2, random_state=2025)

    data_train = np.concatenate((X_train,y_train),axis=1)
    data_eval = np.concatenate((X_test,y_test),axis=1)
    feature_attr = ['d'] * (data_train.shape[1] - 1)
