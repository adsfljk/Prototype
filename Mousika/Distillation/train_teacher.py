...
def select_model(model_name,input_size,output_size):
    if model_name =="gru":
        ...
        return LSTM(input_size, output_size)

def Train_Teacher(X,y,model_name,data_name,min_sample_leaf,args):
    ...
    model = model.cuda()
    BATCH_SIZE = 256
    X_train, X_test, y_train, y_test = train_test_split(X, y, shuffle=True, random_state=2025, test_size=0.2)
    ...
    test_loader = Data.DataLoader(dataset=test_datasets, batch_size=BATCH_SIZE, shuffle=True,)
    # Hyper Parameters
    ...
    model_path = "../params/Teacher_"+str(min_sample_leaf)+data_name+"_model_"+model_name+".pkl"
    ...
    for epoch in range(EPOCH):
        for step, (b_x, b_y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
            ...
            if step % 10 == 0:
                ...
                # print('Epoch: ', epoch, '| train loss: %.4f' % loss.cpu().data.numpy(),'| train acc: %.4f' % np.mean(train_acc), '| test accuracy: %.2f' % accuracy,' best test acc: %.3f' %best_testing_acc)
                train_acc=[]
