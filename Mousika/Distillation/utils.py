...
def accuracy(pred, true_value):
    return np.mean(pred==true_value)
...
def load_data(data_name):
    ...
    features_attr = []
    if data_name=='univ':
        inputName = "./Dataset/univ/univ1-1.csv"
        ...
        #data, drop_data = train_test_split(data, shuffle=True, random_state=20, test_size=0.5)
    if data_name=='univ_C':
        inputName = "./Dataset/univ/univ1-1.csv"
    ...
    if data_name=='univ_test':
        inputName = "./Dataset/univ/univ1-2.csv"
        df = pd.read_csv(inputName)
...
# histogram = {'L': {0: 1313, 1: 4242}, 'R': {0: 1331, 1: 1212}}
#
# l_list = np.array(list(histogram['L'].items()))
# l_num = np.sum(l_list)
# start_time = time.clock()
# for i in range(10707060):
#     y = 5 - cal_entropy_from_histogram(histogram)
# end_time = time.clock()
#
...
def get_c_avg(c):
    ...
    print('macro', m_avg_values)
    m_avg_values.extend(w_avg_values)
    return m_avg_values


# def print_tree(node, parent_id, node_id):
#     if node.label is not None:
#         print(parent_id, node_id, node.label)
#         return
#     else:
#         print(parent_id, node_id, node.feature, node.threshold)
#         node_id+=1
#         print_tree(node.true_branch, node_id, node_id)
#         node_id += 1
#         print_tree(node.false_branch, node_id, node_id)
