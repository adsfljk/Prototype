...
for boost_iter in range(5):
    ...
    model = init_model(model, args.init_type,0.03,2, this_X_train, this_y_train, feature_min, feature_max)
        ...
        for item in cls_fix:
            ...
            for dim1 in range(len(gt_proto)):
                for dim2 in range(len(other_proto)):
                    restrict_cache.append((gt_proto[dim1], other_proto[dim2]))
                    if (other_proto[dim2], gt_proto[dim1]) in restrict: # 反向判断
                        flag = 0
            
            if flag == 0: # 当前规则不能被满足
                res += 1
            else:
                for idx in range(len(restrict_cache)):
                    if restrict_cache[idx] not in restrict:
                        restrict[restrict_cache[idx]] = 1
