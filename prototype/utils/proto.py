...
class prototype(nn.Module):
    ...
    def __init__(self, num_classes, feature_num, prototype_num_classes, temperature, cal_dis, cal_mode):
        ...
        for i in range(self.num_classes):
            ...
            # self.class_prototype.append(nn.Parameter(torch.rand(1, prototype_num_classes[i], self.feature_num).cuda()))
            # self.class_transform.append(nn.Parameter(torch.rand(1, prototype_num_classes[i], self.feature_num).cuda()))
...
def init_model(model, args, init_type,dbscan_eps,min_samples, X_train, y_train, feature_min, feature_max):
    ...
    if init_type == 'NONE':
        ...
        for label in unique_labels:
            ...
            # 使用KMeans进行聚类 len(args.selected_class)
            ...
            # KMeans不产生噪声点，但为了与DBSCAN兼容，这里保持结构一致
